...
const SOLICITUDES_DATA = [
  { id: 'SR-701', host: 'Carlos Ruiz', property: 'Villa de Cristal', service: 'Fotografía Pro', date: '22 Abr, 10:30 AM', price: 'Q 800', status: 'Agendado', avatar: 'C' },
  { id: 'SR-702', host: 'Ana García', property: 'Loft Sky view', service: 'Video Tour 4K', date: '23 Abr, 03:00 PM', price: 'Q 1,500', status: 'Pendiente', avatar: 'A' },
  { id: 'SR-703', host: 'Luis Torres', property: 'Cabaña Bosque', service: 'Pack Completo', date: 'Hoy, 09:15 AM', price: 'Q 4,000', status: 'Completado', avatar: 'L' },
  { id: 'SR-704', host: 'Marta Rivas', property: 'Garden House', service: 'Tour Virtual 360', date: 'Mañana, 11:00 AM', price: 'Q 2,200', status: 'Agendado', avatar: 'M' },
  { id: 'SR-705', host: 'Roberto L.', property: 'Antigua Urban', service: 'Fotografía Pro', date: 'Ayer, 04:45 PM', price: 'Q 800', status: 'Cancelado', avatar: 'R' },
];
...
export default function ServicesScreen() {
  ...
  const getStatusStyle = (status: string) => {
    switch(status) {
      ...
      case 'Agendado': return { text: '#3B82F6', bg: '#EFF6FF', icon: Calendar };
      case 'Pendiente': return { text: '#F59E0B', bg: '#FFFBEB', icon: Clock };
      case 'Cancelado': return { text: '#EF4444', bg: '#FEF2F2', icon: AlertCircle };
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
            {['Todos', 'Pendientes', 'Agendados', 'Completados'].map((label) => {
        ...
        {SOLICITUDES_DATA.map((item, idx) => {
          ...
          return (
            <View key={idx} style={{ 
              ...
              borderBottomWidth: idx === SOLICITUDES_DATA.length - 1 ? 0 : 1, borderBottomColor: '#F8FAFC' 
            }}>
              ...
              <View style={{ flex: 1.8, flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View>
                  <Text style={{ color: C.textDark, fontSize: 14, fontWeight: '800' }}>{item.host}</Text>
                  <Text style={{ color: C.textMuted, fontSize: 12 }}>{item.property}</Text>
                </View>
              </View>
              ...
              <View style={{ flex: 1.5, flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={{ color: C.textDark, fontSize: 13, fontWeight: '600' }}>{item.service}</Text>
              </View>

              {/* Fecha */}
              <View style={{ flex: 1.5 }}>
                <Text style={{ color: C.textMuted, fontSize: 13, fontWeight: '600' }}>{item.date}</Text>
              </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 32, paddingTop: 20, borderTopWidth: 1, borderTopColor: '#F1F5F9' }}>
            <Text style={{ color: C.textMuted, fontSize: 14, fontWeight: '600' }}>Mostrando <Text style={{ color: C.textDark, fontWeight: '800' }}>1 a 5</Text> de 128 solicitudes</Text>
